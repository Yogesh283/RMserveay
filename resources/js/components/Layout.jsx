...
function mobileNavLinkClass(isActive) {
    return [
        'flex min-h-[40px] items-center rounded-lg px-2.5 py-1.5 text-[13px] font-medium leading-snug tracking-tight transition duration-200 ease-out active:scale-[0.99]',
        isActive
            ? 'bg-gradient-to-r from-[#7C3AED]/28 to-[#3B82F6]/16 text-white ring-1 ring-[#F59E0B]/22'
            : 'text-slate-300 hover:bg-white/[0.06] hover:text-white',
    ].join(' ');
}
            ...
            <header className="z-50 border-b border-white/[0.08] bg-[rgba(11,15,26,0.85)] shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-xl max-lg:fixed max-lg:inset-x-0 max-lg:top-0 max-lg:pt-[env(safe-area-inset-top,0px)] lg:sticky lg:top-0 lg:bg-[rgba(11,15,26,0.72)] lg:pt-0">
                ...
                {menuOpen ? (
                    <>
                        ...
                        />
                        <div className="relative z-[51] mx-auto max-h-[min(70vh,calc(100dvh-4.25rem))] overflow-y-auto overscroll-contain rounded-b-2xl border-x border-b border-white/[0.08] border-t border-white/[0.05] bg-gradient-to-b from-[#12182a]/98 via-[#0e1424]/98 to-[#0B0F1A]/98 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 shadow-[0_16px_48px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-xl lg:hidden">
                            <div className="mx-auto mb-2 h-0.5 w-8 shrink-0 rounded-full bg-white/18" aria-hidden />
                            <div className="border-b border-white/[0.06] pb-2">
                                <HomeLanguageSwitcher variant="compact" density="tight" />
                            </div>
                            <div className="mt-2">
                                <p className="mb-1 px-0.5 text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-500">{t('layout.explore')}</p>
                            ...
                            <div>
                                <p className="mb-1 px-0.5 text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-500">{t('layout.account')}</p>
                                <div className="flex flex-col gap-1.5">
                                    {authChecked && user ? (
                                        ...
                                        <div className="grid grid-cols-2 gap-1.5">
                                            <NavLink to="/login" onClick={() => setMenuOpen(false)} className={mobileDrawerBtnLogin}>
                                                {t('layout.login')}
                                            </NavLink>
                                            <NavLink
                                                to={registerMemberTo}
